...
(function(b) {
	...
	var y = function(a, c, d) {
		...
		var e = {
			...
			getRegexMask : function() {
				for (var a = [],
					...
					f = b.optional, ( b = b.recursive) ? (a.push(c.charAt(h)),
					d = {
						digit : c.charAt(h),
						pattern : e
			...
			getMasked : function(a) {
				...
				for (; w(); ) {
					var x = c.charAt(f),
					    ...
					    r = g.translation[x];
					if (r)
						v.match(r.pattern) ? (b[p](v), r.recursive && (-1 === u ? u = f : f === t && ( f = u - m), t === u && (f -= m)), f += m) : r.optional ? (f += m, h -= m) : r.fallback ? (b[p](r.fallback), f += m, h -= m) : e.invalid.push({
							p : h,
							v : v,
							e : r.pattern
						}), h += m;
		...
		g.init = function(c) {
			...
			g = b.extend(!0, {}, g, d);
			l = e.getRegexMask();
			!1 === c ? (d.placeholder && a.attr("placeholder", d.placeholder), b("input").length && !1 === "oninput" in b("input")[0] && "on" === a.attr("autocomplete") && a.attr("autocomplete", "off"), e.destroyEvents(), e.events(),
			c = e.getCaret(), e.val(e.getMasked()), e.setCaret(c + e.getMCharsBeforeCount(c, !0))) : (e.events(), e.val(e.getMasked()))
		};
		g.init(!a.is("input"))
	};
	b.maskWatchers = {};
