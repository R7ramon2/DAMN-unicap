<html>
	...
	<body>
		...
		//$para_faculdade = "h.lumazzini@gmail.com";
		$assunto_faculdade = "Transferencia Externa";
		$corpo_faculdade = "Solicitação de Transferência Externa";
		$corpo_faculdade .= "<br><br><strong>Nome: </strong>$nome";
		$corpo_faculdade .= "<br>Data de nascimento: $data_nasc. Sexo: $sexo";
		...
		$corpo_aluno = "<strong>Confirmação de envio de ficha de transferência externa</strong>";
		...
		if ($arquivo2['size'] > $tamanho) {
			...
			$mail -> SetFrom('secretaria@faculdadeanasps.com.br', 'Solicitação');

			$mail -> AddAddress($para_faculdade, 'Transferencia Externa');
			$mail -> AddCC($para_contato, 'Transferencia Externa');
			...
			if($mail -> Send()){
					...
					$mail -> SetFrom('contato@faculdadeanasps.com.br', 'Ficha Transferência Externa');
				...
				if ($mail -> Send()) {
					...
					echo '<meta HTTP-EQUIV="Refresh" CONTENT="1; URL=../transferencia.html">';
				}
			}
